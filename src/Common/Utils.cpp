...
namespace Utils {
	...
	Renderer::MeshPtr CreatePlaneMesh(float shininess, const std::string& name) {
		Renderer::MeshPtr mesh = std::make_shared<Renderer::Mesh>();
		Renderer::MaterialPtr mat = std::make_shared<Renderer::Material>();
	...
	Renderer::MeshPtr CreateCubeMesh(const std::string& name, float shininess) {
		...
		// Setup mesh: 8 vertices for a cube, each with position, normal, and texture coordinates
		mesh->name = name;
		mesh->vertices = {
			{{-1, -1, -1}},
			{{1, -1, -1}},
