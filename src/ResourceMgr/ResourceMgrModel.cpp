...
namespace ResourceMgr {
    ...
    Renderer::MaterialPtr ProcessMaterial(aiMaterial *aiMaterial, const std::string &directory) {
        ...
        if (aiMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &diffuseTexPath) == AI_SUCCESS) {
            nmat->diffuse = CreateTexture2D(
                directory + getFileName(diffuseTexPath.C_Str()), Renderer::TextureWrapMode::ClampToEdge);
        ...
        if (aiMaterial->GetTexture(aiTextureType_SPECULAR, 0, &specularTexPath) == AI_SUCCESS) {
            nmat->specular = CreateTexture2D(
                directory + getFileName(specularTexPath.C_Str()), Renderer::TextureWrapMode::ClampToEdge);
        ...
        if (aiMaterial->GetTexture(aiTextureType_NORMALS, 0, &normalTexPath) == AI_SUCCESS) {
            nmat->normal =
                CreateTexture2D(directory + getFileName(normalTexPath.C_Str()), Renderer::TextureWrapMode::ClampToEdge);
            nmat->useNormal = true;
        }

        aiString disPlaceTexPath;
        if (aiMaterial->GetTexture(aiTextureType_DISPLACEMENT, 0, &disPlaceTexPath) == AI_SUCCESS) {
            nmat->displacement = CreateTexture2D(
                directory + getFileName(disPlaceTexPath.C_Str()), Renderer::TextureWrapMode::ClampToEdge);
        ...
        if (aiMaterial->GetTexture(aiTextureType_HEIGHT, 0, &heightTexPath) == AI_SUCCESS) {
            nmat->displacement =
                CreateTexture2D(directory + getFileName(heightTexPath.C_Str()), Renderer::TextureWrapMode::ClampToEdge);
            nmat->useDisplacement = true;
        }
    ...
    void ProcessNode(
        ...
        for (int i = 0; i < node->mNumChildren; i++)
            ProcessNode(node->mChildren[i], scene, global_transform, model, model_dir);
    }
    ...
    bool LoadModel(const std::string &name, const std::string &path, AssetManager::ShaderHandle shader) {
        ...
        for (const auto &mat : g_materials | std::views::values) mat->shader = shader;
