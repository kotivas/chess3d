...
namespace Logger {
    enum class Severity : uint8_t { Fatal = 0, Error, Warning, Info, Debug };
    ...
    class Logger final {
    public:
        Logger() : _severity(Severity::Debug) {} // log everything
...
namespace Log {
    ...
    template <typename... Args> void Fatal(const std::string_view fmt, Args &&...args) {
        g_logger->log(Logger::Severity::Fatal, fmt, std::make_format_args(std::forward<Args>(args)...));
    }

    template <typename... Args> void Log(const Logger::Severity sev, const std::string_view fmt, Args &&...args) {
