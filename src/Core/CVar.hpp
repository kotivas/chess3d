...
namespace CVar {
    struct CVar {
        using CVarValue =
            std::variant<float, bool, int, std::array<float, 2>, std::array<float, 3>, std::array<float, 4>>;
        ...
        CVar(
            ...
            int min = 0,
            int max = 0) :
            name(std::move(name)), desc(std::move(desc)), val(value), defVal(value), minFloat(static_cast<float>(min)),
            maxFloat(static_cast<float>(max)), cb(std::move(onChange)) {}
        ...
        CVar(
            ...
            CVarCb on_change,
            std::string desc = "No description provided") :
            name(std::move(name)), desc(std::move(desc)), val(value), defVal(value), cb(std::move(on_change)) {}
        ...
        template <typename T> void set(T &new_val) {
            T clamped = new_val;
            ...
            if constexpr (std::is_same_v<T, int>) {
                ...
                for (auto &elem : clamped) elem = clampVal(elem);
            }
