...
namespace CMDUtils {
    void Execute(const std::string &name, const std::string &args) {
        ...
        std::visit(
            [&]<typename T>(T &current) {
                if (auto parsed = Utils::TryParse<T>(args))
                    cvar->set(*parsed);
                else
    ...
    // --- float ---
    void Register(const std::string &name, const std::string &desc, float &target, float min, float max) {
        Register(
            CVar::CVar{
                name, target, [&target](const CVar::CVar &cvar) { target = std::get<float>(cvar.val); }, desc, min,
                max});
    }

    // --- int ---
    void Register(const std::string &name, const std::string &desc, int &target, int min, int max) {
        Register(
            CVar::CVar{
                name, target, [&target](const CVar::CVar &cvar) { target = std::get<int>(cvar.val); }, desc, (min),
                (max)});
    }
    ...
    void Register(const std::string &name, const std::string &desc, bool &target) {
        Register(
            CVar::CVar{name, target, [&target](const CVar::CVar &cvar) { target = std::get<bool>(cvar.val); }, desc});
    }
    ...
    void Register(const std::string &name, const std::string &desc, std::array<float, 2> &target) {
        Register(
            CVar::CVar{
                name, target, [&target](const CVar::CVar &cvar) { target = std::get<std::array<float, 2>>(cvar.val); },
                desc});
    }
    ...
    void Register(const std::string &name, const std::string &desc, std::array<float, 3> &target) {
        Register(
            CVar::CVar{
                name, target, [&target](const CVar::CVar &cvar) { target = std::get<std::array<float, 3>>(cvar.val); },
                desc});
    }
    ...
    void Register(const std::string &name, const std::string &desc, std::array<float, 4> &target) {
        Register(
            CVar::CVar{
                name, target, [&target](const CVar::CVar &cvar) { target = std::get<std::array<float, 4>>(cvar.val); },
                desc});
    }
    ...
    void Register(const std::string &name, const std::string &desc, glm::vec3 &target) {
        Register(
            CVar::CVar{
                name, std::array{target.x, target.y, target.z},
    ...
    void Register(const std::string &name, const std::string &desc, glm::vec2 &target) {
        Register(
            CVar::CVar{
                name, std::array{target.x, target.y},
                [&target](const CVar::CVar &cvar) {
    ...
    void Register(const std::string &name, const std::string &desc, glm::vec4 &target) {
        Register(
            CVar::CVar{
                name, std::array{target.x, target.y, target.z, target.w},
                [&target](const CVar::CVar &cvar) {
        ...
        return std::visit(
            [](auto &&arg) -> std::string {
                ...
                if constexpr (std::is_same_v<T, bool>) {
                    ...
                    std::is_same_v<T, std::array<float, 2>> || std::is_same_v<T, std::array<float, 3>>
                    || std::is_same_v<T, std::array<float, 4>>) {
                    return Utils::ArrayToString(arg);
                } else {
    ...
    CVar::cvar_t *Find(const std::string &name) {
        ...
        if (it != CVar::g_cvars.end()) return &it->second;
        return nullptr;
    }
} // namespace CMDUtils
