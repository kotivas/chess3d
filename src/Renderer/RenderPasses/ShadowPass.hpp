...
namespace Renderer {
    struct ShadowData {
        ShadowData() : resolution(0), map(0), fbo(0), light_space(0), shader() {}
    ...
    struct OmniShadowData {
        OmniShadowData() : resolution(0), cubemap(0), fbo(0), transforms(), shader() {}
    ...
    private:
        ...
        glm::mat4
        calcDirLightSpace(const glm::vec3 &light_dir, const Camera::CameraInfo &cam, float shadow_distance) const;
        glm::mat4 calcSpotLightSpace(
            const glm::vec3 &light_pos, const glm::vec3 &light_dir, float fov, float near, float far) const;
        std::array<glm::mat4, 6>
        calcPointLightSpace(const glm::vec3 &light_pos, float near, float far, float resolution) const;
