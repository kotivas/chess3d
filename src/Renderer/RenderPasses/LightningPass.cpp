...
namespace Renderer {
    ...
    void LightningPass::createUboLights() {
        ...
        glBufferData(
            GL_UNIFORM_BUFFER, sizeof(DirLight) + sizeof(PointLight) + sizeof(SpotLight), NULL, GL_STATIC_DRAW);
        glBindBuffer(GL_UNIFORM_BUFFER, 0);

        glBindBufferRange(
            GL_UNIFORM_BUFFER, 1, _ubo_lights, 0, sizeof(DirLight) + sizeof(PointLight) + sizeof(SpotLight));
    }

    void LightningPass::updateUboMatrices(
        ...
        glBufferSubData(
            GL_UNIFORM_BUFFER, 2 * sizeof(glm::mat4), sizeof(glm::mat4), glm::value_ptr(dir_light_space_matrix));
        glBufferSubData(
            GL_UNIFORM_BUFFER, 3 * sizeof(glm::mat4), sizeof(glm::mat4), glm::value_ptr(spot_light_space_matrix));

        glBindBuffer(GL_UNIFORM_BUFFER, 0);
    }

    void LightningPass::updateUboLights(
        const DirLight &dir_light, const PointLight &point_light, const SpotLight &spot_light) const {
        glBindBuffer(GL_UNIFORM_BUFFER, _ubo_lights);
    ...
    void LightningPass::pass(const RenderContext &ctx) const {
        ...
        updateUboMatrices(
            ctx.camera.projectionMatrix, ctx.camera.viewMatrix, ctx.dir_shadow.light_space,
            ctx.spot_shadow.light_space);
