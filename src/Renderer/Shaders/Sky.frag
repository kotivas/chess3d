...
void main() {
    ...
    // Sun disk
    float sunAmount = smoothstep(0.995, 1.0, dot(normalize(rayDir), sunDir));
    skyColor += sunColor * sunAmount;

    // Cirrus clouds
    float cirrusDensity = smoothstep( 1.0 - cirrus, 1.0, fbm(rayDir * 4.0 + time * 0.15)) * 0.3;
    skyColor = mix(skyColor, vec3(1.0), cirrusDensity*rayDir.y);

    // Cumulus clouds
    for(int i=0;i<3;i++){
        float cumulusDensity = smoothstep(1.0 - cumulus, 1.0, fbm(rayDir * 3.0 + vec3(i*0.01) + time * 0.6));
