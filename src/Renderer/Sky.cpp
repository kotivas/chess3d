...
namespace Renderer {
    ...
    void Sky::setup() {
        const float quadVertices[] = {-1.f, 1.f, -1.f, -1.f, 1.f, 1.f, 1.f, -1.f};
    ...
    inline float EvaluateSpline(const float *dataset, std::size_t start, std::size_t stride, float value) {
        ...
        return (inv * inv * inv * inv * inv) * dataset[start + 0 * stride]
    ...
    inline float Evaluate(const float *dataset, std::size_t stride, float turbidity, float albedo, float sun_theta) {
        ...
        return a0t0 * (1.0f - albedo) * (1.0f - turbidityK) + a1t0 * albedo * (1.0f - turbidityK)
            + a0t1 * (1.0f - albedo) * turbidityK + a1t1 * albedo * turbidityK;
    }
    ...
    glm::vec3 HosekWilkie(float cos_theta, float gamma, float cos_gamma, const std::array<glm::vec3, 10> &params) {
        ...
        glm::vec3 second = C + D * glm::exp(E * gamma) + glm::vec3(F * (cos_gamma * cos_gamma)) + G * chi
            + I * glm::sqrt(glm::max(glm::vec3(0.0f), glm::vec3(cos_theta)));
    ...
    void Sky::calculateSun(const glm::vec2 &sun_pos) {
        ...
        // channel 0..2
        const float *datasets[3] = {Hosek::DATASET_RGB1.data(), Hosek::DATASET_RGB2.data(), Hosek::DATASET_RGB3.data()};
        ...
        for (int c = 0; c < 3; ++c) {
            ...
            atmParams[0][c] = Evaluate(ds + 0, 9, atmTurbidity, ALBEDO[c], sun_theta);
            atmParams[1][c] = Evaluate(ds + 1, 9, atmTurbidity, ALBEDO[c], sun_theta);
            atmParams[2][c] = Evaluate(ds + 2, 9, atmTurbidity, ALBEDO[c], sun_theta);
            atmParams[3][c] = Evaluate(ds + 3, 9, atmTurbidity, ALBEDO[c], sun_theta);
            atmParams[4][c] = Evaluate(ds + 4, 9, atmTurbidity, ALBEDO[c], sun_theta);
            atmParams[5][c] = Evaluate(ds + 5, 9, atmTurbidity, ALBEDO[c], sun_theta);
            atmParams[6][c] = Evaluate(ds + 6, 9, atmTurbidity, ALBEDO[c], sun_theta);
            atmParams[7][c] = Evaluate(ds + 8, 9, atmTurbidity, ALBEDO[c], sun_theta); // swapped
            atmParams[8][c] = Evaluate(ds + 7, 9, atmTurbidity, ALBEDO[c], sun_theta);

            // RAD dataset
            atmParams[9][c] = Evaluate(datasets_rad[c], 1, atmTurbidity, ALBEDO[c], sun_theta);
        }

        glm::vec3 S = HosekWilkie(std::cos(sun_theta), 0.0f, 1.0f, atmParams) * atmParams[9];
